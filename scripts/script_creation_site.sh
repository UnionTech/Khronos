...
#***************************CREATION DU FQDN*************************************
echo "fqdn" > /var/www/scripts/a.txt
IP=88.177.168.133
...
echo "vhost" >> /var/www/scripts/a.txt
...
echo "avant index">>/var/www/scripts/a.txt
./index.sh $nom_domaine

echo "après index">>/var/www/scripts/a.txt
#****Création du fichier du vhost****
cd /etc/nginx/sites-available/ >/dev/null 2>&1
touch $nom_domaine >/dev/null 2>&1
echo "après touch">>/var/www/scripts/a.txt
...
echo "après remplissage vhost">>/var/www/scripts/a.txt
cd - >/dev/null 2>&1
...
echo "avant reload">>/var/www/scripts/a.txt
service nginx reload >/dev/null 2>&1
service php5-fpm reload >/dev/null 2>&1
echo "après reload">>/var/www/scripts/a.txt
...
#tmpReel=$(($tmp+168))
tmpReel=$(($tmp+5))
echo "après tmp réel">>/var/www/scripts/a.txt

#****Mise en place du htpasswd dans le script test****
at now +$tmp hours <<FIN
./test.sh $nom_domaine
FIN
echo "après premier at">>/var/www/scripts/a.txt
